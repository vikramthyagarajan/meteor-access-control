...
EntityInstanceManager = class {
	...
	syncDatabase() {
		if(Meteor.isServer)
			Meteor.publish('aclEntityInstances', () => {
				return this.entityInstances.find({});
			});
		else {
			Meteor.subscribe('aclEntityInstances');
		}
	}
	/**
	...
	addEntityInstance(entity, instanceObj, allEntities, allObjects) {
		...
		//if entity with same instance exists, then ignoring
		if(!this.entityInstances.findOne({entity: entity._id,instanceId: id})) {
