...
EntityManager = class {
	...
	canInstancePerform(entityName, entityObj, action, actionType, objectType, objectId) {
		...
		var ruleObj = this.entityInstanceManager.getRuleOfInstance(instanceId, 
			entity._id, actionType);
		var capability, args;
		if(ruleObj){
			capability = ruleObj.capability;
			args = ruleObj.args;
		}
		else
	 ...
	 */
	setCapabilityOfInstance(entityName, entityInstanceId, action, objectType, capabilityName) {
		this.entityInstanceManager.setCapabilityOfInstance(entityInstanceId, action, objectType,
			capabilityName);
	}
	/**
