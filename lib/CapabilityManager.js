...
CapabilityManager = class {
	...
	syncDatabase() {
		if(Meteor.isServer)
			Meteor.publish('aclCapabilities', () => {
				return this.capabilities.find({});
			});
		else {
			Meteor.subscribe('aclCapabilities');
		}
	}
	/**
	...
	getCapability(capabilityName) {
		return this.capabilities.findOne({name: capabilityName});
	}
	/**
	 * Gets a specific capability by Id
	 */
	getCapabilityById(capabilityId) {
		return this.capabilities.findOne({_id: capabilityId});
	}
	/**
